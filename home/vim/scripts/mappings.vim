...
function! DuplicateText(type) abort
...
" @wincent is very smart - https://www.youtube.com/watch?v=0aEv1Nj0IPg
...
command! InfoWindowCustomToggle call infowindow#toggle({} , { default_lines -> extend(default_lines, [
  \['branch', " " . fugitive#head()[0:15]],
  \['filetype', &filetype],
  \['syntax', &syntax],
  \['path', expand(@%)]
\]) })
...
" mappings {{{ "
  ...
  " normal mode {{{ "
    ...
    " local leader {{{ "
      ...
      nmap <silent> <LocalLeader><LocalLeader> :Vista finder fzf<CR>
      nmap <silent> <LocalLeader>[ :Vista!!<CR>
      ...
      map <silent> <LocalLeader>x :call checkbox#ToggleCB()<cr>
...
" Edit dotfiles {{{ "
  command! Emap      execute "e $DOTFILES/home/vim/scripts/mappings.vim"
  command! Eauto     execute "e $DOTFILES/home/vim/scripts/autocommands.vim"
  command! Eplugin   execute "e $DOTFILES/home/vim/scripts/plugin_settings.vim"
  command! Ecomp     execute "e $DOTFILES/home/vim/scripts/completions.vim"
  ...
  map <silent> <leader>ea :Eauto<cr>
  map <silent> <leader>ec :Ecomp<cr>
  map <silent> <leader>ed :Edotfiles<cr>
  map <silent> <leader>ee :Ezshenv<cr>
  map <silent> <leader>ef :Efunction<cr>
  map <silent> <leader>eg :execute 'normal viq"gy' <bar> :call LuckySearch()<CR>
  map <silent> <leader>ei :PlugInstall<cr>
  map <silent> <leader>em :Emap<cr>
  map <silent> <leader>ep :Eplugin<cr>
  map <silent> <leader>esd :EditFileTypeDetect<cr>
  map <silent> <leader>esp :EditFileTypePlugin<cr>
  map <silent> <leader>et :Etmux<cr>
  map <silent> <leader>eu :PlugUpdate<cr>
  map <silent> <leader>ev :e $MYVIMRC<cr>
  map <silent> <leader>ez :Ezshrc<cr>
" }}} Edit dotfiles "
